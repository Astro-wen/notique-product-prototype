  ...
  function Sidebar() {
    ...
    return (
      <aside className="sidebar">
        ...
        <nav>
          <button className={view === "templates" && templateMode === "AI templates" ? "nav-item active" : "nav-item"} onClick={() => openTemplate("AI templates")}><span>▤</span><span>{t("Templates", "範本")}</span></button>
        ...
        <div className="folder-grid">
          {ordered.map((folder) => (
            <article className="folder-card" key={folder.id} role="button" tabIndex={0} onClick={() => openWorkspace(folder.id)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") openWorkspace(folder.id) }}>
              ...
              {folder.id === "product" || folder.id === "oak" ? <span className="update-badge">{folder.kind === "contractor" ? 3 : 3} {t("updates", "項更新")}</span> : null}
  ...
  function FolderView() {
    ...
    return (
      <div className="product-page">
        ...
        <div className="folder-section-heading"><div><h2>{t("Files and recordings", "檔案與錄音")}</h2><p>{t("Everything saved to this Project, including items filed by AI.", "這個專案裡的所有內容，包括 AI 歸檔的項目。")}</p></div><button className="sort" onClick={() => setSortNewest((value) => !value)}>{sortNewest ? t("Newest first⌄", "最新項目⌄") : t("Oldest first⌃", "最早項目⌃")}</button></div>
  ...
  function AgentView() {
    ...
    return (
      <div className="agent-page">
        ...
        <div className="agent-title"><span className="agent-mark">✦</span><div><h1>{t("Where should this go?", "這個內容應該放在哪裡？")}</h1><p>{t("Notique checks the item and your recent project activity. You make the final choice.", "Notique 會檢查內容與最近的專案活動，由你做最後決定。")}</p></div></div>
        ...
        {!agentReady ? <section className="analyzing"><span className="spinner" /><h2>{t("Finding the best project", "正在尋找最適合的專案")}</h2><p>{t("Reading the title, available content and recent project context.", "正在讀取標題、內容與最近的專案背景。")}</p></section> : <><section className="suggestion-section"><header><div><h2>{t("Two likely projects", "兩個可能的專案")}</h2><p>{t("Select one, then allow Notique to file the item.", "選擇一個專案，再允許 Notique 歸檔。")}</p></div><span>{t("AI suggestion", "AI 建議")}</span></header><div className="suggestions">{suggestions.map((suggestion, index) => { const folder = folders.find((item) => item.id === suggestion.folderId)!; const active = selectedSuggestion === suggestion.folderId; return <button className={active ? "suggestion active" : "suggestion"} key={suggestion.folderId} onClick={() => setSelectedSuggestion(suggestion.folderId)}><span className="radio">{active ? "●" : ""}</span><span className={`mini-folder ${folder.color}`}>▰</span><span className="suggestion-copy"><span><b>{index === 0 ? t("BEST MATCH", "最符合") : t("ALSO POSSIBLE", "也有可能")}</b><em>{suggestion.confidence}% {t("match", "符合")}</em></span><strong>{folder.name}</strong><small>{folder.description}</small><ul>{suggestion.reasons.map((reason) => <li key={reason}>{reason}</li>)}</ul></span></button> })}</div></section><section className="agent-confirm"><span><strong>{t("File in", "歸檔到")} {folders.find((folder) => folder.id === selected?.folderId)?.name}</strong><small>{t("You can move or rename it later.", "之後仍可移動或重新命名。")}</small></span><button className="text-button" onClick={keepInInbox}>{t("Keep in Inbox", "保留在收件匣")}</button><button className="primary allow" onClick={allowFiling}>{t("Allow and file", "允許並歸檔")}</button></section></>}
      </div>
    );
  }
